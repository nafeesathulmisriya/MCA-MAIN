...
def upload_claim(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            claim.user = request.user  # Ensure user is assigned
            claim.save()  
            messages.success(request, "Your claim has been submitted successfully.")
            return redirect(reverse("claim_success") + f"?policy_number={claim.policy_number}")  
        else:
...
def claim_success(request):
    policy_number = request.GET.get("policy_number", "N/A")  # Get policy number from URL
    return render(request, "customer/claim_success.html", {"policy_number": policy_number})
